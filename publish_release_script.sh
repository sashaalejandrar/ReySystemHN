#!/bin/bash
# Script para publicar release - versión mejorada sin conflictos de librerías

RELEASE_ID=$1
DB_NAME="tiendasrey"
DB_USER="root"
DB_PASS=""

# CRÍTICO: Usar binarios del sistema, NO de XAMPP
export PATH=/usr/bin:/bin:/usr/local/bin:$PATH
unset LD_LIBRARY_PATH
unset LD_PRELOAD

# Cambiar al directorio del proyecto
cd /opt/lampp/htdocs/ReySystem

echo "=== Publicando Release ID: $RELEASE_ID ==="

# Configurar safe.directory
git config --global --add safe.directory /opt/lampp/htdocs/ReySystem 2>/dev/null || true

# Obtener datos de la release
QUERY="SELECT version, codename, build, release_date, changes_json, file_path FROM updates WHERE id=$RELEASE_ID"
RELEASE_DATA=$(/opt/lampp/bin/mysql -u $DB_USER $DB_NAME -N -e "$QUERY")

if [ -z "$RELEASE_DATA" ]; then
    echo "ERROR: Release no encontrada"
    exit 1
fi

# Parsear datos
VERSION=$(echo "$RELEASE_DATA" | cut -f1)
CODENAME=$(echo "$RELEASE_DATA" | cut -f2)
BUILD=$(echo "$RELEASE_DATA" | cut -f3)
RELEASE_DATE=$(echo "$RELEASE_DATA" | cut -f4)
CHANGES_JSON=$(echo "$RELEASE_DATA" | cut -f5)
FILE_PATH=$(echo "$RELEASE_DATA" | cut -f6)

GIT_TAG="v$VERSION"

echo "Versión: $VERSION"
echo "Codename: $CODENAME"
echo "Tag: $GIT_TAG"

# Leer token desde .env
if [ ! -f .env ]; then
    echo "ERROR: Archivo .env no encontrado"
    exit 1
fi

GH_TOKEN=$(grep "^GH_TOKEN=" .env | cut -d= -f2)

if [ -z "$GH_TOKEN" ]; then
    echo "ERROR: No se encontró GH_TOKEN en .env"
    exit 1
fi

echo "Token encontrado: ${GH_TOKEN:0:15}..."

# Configurar usuario de Git
git config user.name "ReySystem Bot" 2>/dev/null || true
git config user.email "reysystem@localhost" 2>/dev/null || true

# Verificar si version.json tiene cambios
echo "Verificando cambios en version.json..."
git add version.json 2>/dev/null

# Verificar si hay cambios para commitear
if git diff --cached --quiet; then
    echo "No hay cambios en version.json, usando commit actual"
    COMMIT_SHA=$(git rev-parse HEAD)
else
    # Hay cambios, hacer commit
    echo "Commiteando cambios en version.json..."
    COMMIT_MSG="Release $GIT_TAG - $CODENAME"
    git commit -m "$COMMIT_MSG"
    if [ $? -ne 0 ]; then
        echo "ERROR: git commit falló"
        exit 1
    fi
    COMMIT_SHA=$(git rev-parse HEAD)
fi

echo "Commit SHA: $COMMIT_SHA"

# Crear/actualizar tag
echo "Creando tag $GIT_TAG..."
git tag -d $GIT_TAG 2>/dev/null || true
git tag -a $GIT_TAG -m "$CODENAME"
if [ $? -ne 0 ]; then
    echo "ERROR: No se pudo crear tag"
    exit 1
fi

# Push a GitHub usando git push (más simple y confiable)
echo "Subiendo a GitHub..."
PUSH_URL="https://sashaalejandrar:$GH_TOKEN@github.com/sashaalejandrar/ReySystemHN.git"

# Push main
git push "$PUSH_URL" main 2>&1
if [ $? -ne 0 ]; then
    echo "WARNING: Push main falló, continuando..."
fi

# Push tag (forzado para sobrescribir si existe)
git push -f "$PUSH_URL" $GIT_TAG 2>&1
if [ $? -ne 0 ]; then
    echo "ERROR: No se pudo subir tag a GitHub"
    exit 1
fi

echo "✓ Tag subido a GitHub"

# Crear release en GitHub con gh CLI
if command -v gh &> /dev/null; then
    echo "Creando release en GitHub con gh CLI..."
    
    # Crear notas desde changes_json
    NOTES_FILE=$(mktemp)
    echo "## 🎉 Novedades" > $NOTES_FILE
    echo "" >> $NOTES_FILE
    
    # Parsear JSON de cambios
    echo "$CHANGES_JSON" | sed 's/\[//g' | sed 's/\]//g' | sed 's/"//g' | tr ',' '\n' | while read line; do
        if [ ! -z "$line" ]; then
            echo "- $line" >> $NOTES_FILE
        fi
    done
    
    echo "" >> $NOTES_FILE
    echo "## 📦 Instalación" >> $NOTES_FILE
    echo "Descarga el archivo adjunto y extrae en tu servidor." >> $NOTES_FILE
    
    # Configurar variables de entorno para gh
    export GH_TOKEN=$GH_TOKEN
    export GITHUB_TOKEN=$GH_TOKEN
    export GH_HOST=github.com
    
    # Crear directorio temporal para config de gh (evitar problemas de permisos)
    GH_CONFIG_DIR=$(mktemp -d)
    export GH_CONFIG_DIR=$GH_CONFIG_DIR
    export HOME=$GH_CONFIG_DIR
    
    # Título de la release
    TITLE="$GIT_TAG - $CODENAME"
    
    # Verificar si la release ya existe
    gh release view $GIT_TAG 2>/dev/null
    if [ $? -eq 0 ]; then
        echo "Release ya existe, eliminándola..."
        gh release delete $GIT_TAG -y 2>/dev/null || true
    fi
    
    # Crear release
    if [ -f "$FILE_PATH" ]; then
        echo "Subiendo con archivo: $FILE_PATH"
        gh release create $GIT_TAG --title "$TITLE" --notes-file $NOTES_FILE "$FILE_PATH" 2>&1
    else
        echo "Sin archivo adjunto"
        gh release create $GIT_TAG --title "$TITLE" --notes-file $NOTES_FILE 2>&1
    fi
    
    GH_CODE=$?
    
    # Limpiar
    rm -f $NOTES_FILE
    rm -rf $GH_CONFIG_DIR
    
    if [ $GH_CODE -eq 0 ]; then
        echo "✓ Release creada en GitHub"
        
        # Obtener URL
        RELEASE_URL=$(gh release view $GIT_TAG --json url -q .url 2>/dev/null)
        
        if [ ! -z "$RELEASE_URL" ]; then
            echo "URL: $RELEASE_URL"
            # Actualizar BD con URL
            /opt/lampp/bin/mysql -u $DB_USER $DB_NAME -e "UPDATE updates SET status='published', published_at=NOW(), github_tag='$GIT_TAG', github_release_url='$RELEASE_URL' WHERE id=$RELEASE_ID"
        else
            # Actualizar BD sin URL
            /opt/lampp/bin/mysql -u $DB_USER $DB_NAME -e "UPDATE updates SET status='published', published_at=NOW(), github_tag='$GIT_TAG' WHERE id=$RELEASE_ID"
        fi
    else
        echo "ERROR: No se pudo crear release en GitHub (código: $GH_CODE)"
        # Actualizar BD de todas formas
        /opt/lampp/bin/mysql -u $DB_USER $DB_NAME -e "UPDATE updates SET status='published', published_at=NOW(), github_tag='$GIT_TAG' WHERE id=$RELEASE_ID"
        exit 1
    fi
else
    echo "WARNING: GitHub CLI no instalado"
    # Actualizar BD sin GitHub
    /opt/lampp/bin/mysql -u $DB_USER $DB_NAME -e "UPDATE updates SET status='published', published_at=NOW(), github_tag='$GIT_TAG' WHERE id=$RELEASE_ID"
fi

echo ""
echo "=== ✓ Release publicada exitosamente ==="
exit 0
