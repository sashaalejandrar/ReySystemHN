      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex gap-2">
            <button type="button" onclick="loadRecentChanges()" id="loadChangesBtn"
            ...
            <button type="button" onclick="generateAIChangelog()" id="aiChangelogBtn"
              class="text-xs px-3 py-1 bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white rounded-lg transition-colors flex items-center gap-1 font-semibold">
              <span class="material-symbols-outlined text-sm">psychology</span>
              IA Groq
            </button>
          </div>
        </div>
        <textarea name="changes" id="changesTextarea" required rows="6" placeholder="Nueva característica X&#10;Mejora en Y&#10;Fix en Z"
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1" id="changesHint">Cada línea será un cambio en el changelog</p>
      </div>
...
function openNewReleaseModal() {
    ...
    // Auto-cargar cambios al abrir el modal
    loadRecentChanges();
}
...
async function loadRecentChanges() {
    ...
    try {
        // Mostrar loading
        btn.disabled = true;
        btn.innerHTML = '<span class="material-symbols-outlined text-sm animate-spin">refresh</span> Cargando...';
        hint.textContent = 'Obteniendo cambios desde Git...';
        hint.className = 'text-xs text-blue-500 dark:text-blue-400 mt-1';
        
        const response = await fetch('api_releases.php?action=get_recent_changes');
        const data = await response.json();
        
        if (data.success && data.changes && data.changes.length > 0) {
            // Llenar textarea con los cambios
            textarea.value = data.changes.join('\n');
            
            // Mostrar mensaje de éxito
            const sourceMsg = data.source === 'git' 
                ? `${data.total_commits} commits desde ${data.last_version || 'inicio'}`
                : data.source === 'files'
                ? 'Basado en archivos modificados'
                : 'Sugerencias por defecto';
            
            hint.textContent = `✓ ${data.changes.length} cambios cargados (${sourceMsg})`;
            hint.className = 'text-xs text-green-600 dark:text-green-400 mt-1';
        } else {
            throw new Error(data.message || 'No se encontraron cambios');
        }
        
    } catch (error) {
        console.error('Error loading changes:', error);
        hint.textContent = '⚠ No se pudieron cargar cambios automáticamente. Escribe manualmente.';
        hint.className = 'text-xs text-orange-500 dark:text-orange-400 mt-1';
    } finally {
        // Restaurar botón
        btn.disabled = false;
        btn.innerHTML = originalBtnText;
        
        // Restaurar hint después de 3 segundos
        setTimeout(() => {
            hint.textContent = 'Cada línea será un cambio en el changelog';
...
async function generateAIChangelog() {
    ...
    try {
        // Mostrar loading
        btn.disabled = true;
        btn.innerHTML = '<span class="material-symbols-outlined text-sm animate-spin">psychology</span> Generando con IA...';
        hint.textContent = '🤖 Groq AI está analizando los cambios...';
        hint.className = 'text-xs text-orange-500 dark:text-orange-400 mt-1 font-medium';
        
        const response = await fetch('api_releases.php?action=generate_ai_changelog');
        const data = await response.json();
        
        if (data.success && data.changes && data.changes.length > 0) {
            // Llenar textarea con los cambios generados por IA
            textarea.value = data.changes.join('\n');
            
            // Mostrar mensaje de éxito
            hint.textContent = `✨ ${data.changes.length} cambios generados por ${data.model || 'IA'} - ¡Edita si es necesario!`;
            hint.className = 'text-xs text-green-600 dark:text-green-400 mt-1 font-medium';
        } else {
            throw new Error(data.message || 'No se pudieron generar cambios con IA');
        }
        
    } catch (error) {
        console.error('Error generating AI changelog:', error);
        hint.textContent = '⚠ Error con IA. Intenta con Auto-generar o escribe manualmente.';
        hint.className = 'text-xs text-red-500 dark:text-red-400 mt-1';
    } finally {
        // Restaurar botón
        btn.disabled = false;
        btn.innerHTML = originalBtnText;
        
        // Restaurar hint después de 5 segundos
        setTimeout(() => {
            hint.textContent = 'Cada línea será un cambio en el changelog';
            hint.className = 'text-xs text-slate-500 dark:text-slate-400 mt-1';
        }, 5000);
