...
switch ($metodo) {
    ...
    case 'POST':
        ...
        $check = $conexion->prepare("SELECT id_marca FROM Marcas WHERE nombre = ?");
        ...
        $stmt = $conexion->prepare("INSERT INTO Marcas (nombre, descripcion, creado_por) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $nombre, $descripcion, $creado_por);
        
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Marca creada exitosamente', 'id' => $conexion->insert_id]);
        } else {
    ...
    case 'PUT':
        ...
        $check = $conexion->prepare("SELECT id_marca FROM Marcas WHERE nombre = ? AND id_marca != ?");
        ...
        $stmt = $conexion->prepare("UPDATE Marcas SET nombre = ?, descripcion = ? WHERE id_marca = ?");
