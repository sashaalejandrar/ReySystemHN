...
async function cargarAnalisis() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-8 text-center"><div class="flex items-center justify-center gap-2"><span class="material-symbols-outlined animate-spin">refresh</span><span>Cargando análisis...</span></div></td></tr>';
    
    console.log('🔍 Cargando análisis ABC...');
    console.log('Fecha inicio:', fechaInicio);
    console.log('Fecha fin:', fechaFin);
    
    try {
        const url = `api/get_analisis_abc.php?fecha_inicio=${fechaInicio}&fecha_fin=${fechaFin}`;
        ...
        if (result.success) {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="px-6 py-8 text-center text-red-500">Error: ${result.message || 'No se pudo cargar el análisis'}</td></tr>`;
        }
    } catch (error) {
